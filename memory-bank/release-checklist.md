...
Add a new `## [X.Y.Z] — YYYY-MM-DD` section at the top of `CHANGELOG.md` with all changes
grouped under `### Fixed`, `### Added`, `### Changed`, and `### Credits` (if applicable).

> **Critical:** This MUST happen before tagging. The release pipeline extracts the changelog
> section for the GitHub Release body via `awk`. If missing, the release has no description
> and the Discord notification will be empty.
...
### 6. Wait for CI (all must pass before Release pipeline completes)
```bash
...
### 7. Release Pipeline (AUTOMATIC — triggered by tag push)
...
### 8. Update AUR (lean-ctx — source build)
```bash
...
### 9. Update AUR (lean-ctx-bin — pre-built binary)
```bash
...
### 10. Commit AUR changes to main repo
```bash
cd /path/to/lean-ctx
...
### 12. Post Discord Release Announcement

Post release notes to the Discord `#releases` channel. Include key changes, upgrade
instructions, and credits for community contributors.
