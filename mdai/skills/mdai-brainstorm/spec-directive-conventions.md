...
| Sub-Render mit args (NEW v1.0)         | `@render-template from="<src>" to="<dst>" [force]` + key=value body + `@end` | manual string-concat in @query      | `@render-template`                        |
| Conditional anchor-check (NEW v1.0)    | `@if file.containsLine "<file>" "<anchor>"`                                  | grep-output check                   | `file.containsLine`                       |
| Iteration über list/anchors (NEW v1.0) | `@foreach item in items` + body + `@end`                                     | repeated `@if`-Blöcke               | `@foreach` + `@set`                       |
| Multi-branch (NEW v1.0)                | `@switch <var>` + `@case "..."` + `@default` + `@endswitch`                  | repeated `@if`/`@elseif`-chain      | `@switch`/`@case`/`@default`              |
