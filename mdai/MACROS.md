...
Quelle der Wahrheit für alle versionierten mdai-Macros. Konsumiert von den drei mdai-Skills (`mdai-brainstorm`,
`mdai-execution`, `mdai-memory`) und jedem generierten `.mdai.md`-Plan via `@call mdai_bootstrap()` + `@import`/
`@include`.
...
- **Bootstrap:** Jeder konsumierende Skill ruft `@call mdai_bootstrap()` als erste Zeile in `pre-context`. Setzt
  `ctx_session`-Flags für MCP-Liveness + Projekt-Typ + Tooling.
...
**Hinweis Skill A:** Skill-A-Spec (`docs/mdai/specs/2026-05-23-mdai-brainstorm-design.mdai.md`) MUSS in separater
Patch-Session aktualisiert werden, BEVOR Skill-A-A1 (Impl-Start) läuft — siehe library-spec §10. Skill A ist nach diesem
Release render-broken bis Patch-Session durch ist (beabsichtigt, A9-Cleanup-Entscheidung).
