...
| Read file (bounded line range)       | `@call ctx_read_lines(path, start, end)`           | `mcp__lean-ctx__ctx_read mode="lines:N-M"`                                   |
| Read file (deps + cross-refs scan)   | `@call ctx_read_map(path)`                         | `mcp__lean-ctx__ctx_read mode="map"`                                         |
| Read file (API surface scan)         | `@call ctx_read_signatures(path)`                  | `mcp__lean-ctx__ctx_read mode="signatures"`                                  |
