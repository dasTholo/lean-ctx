...
fn gate_header_include_read_render_e2e() {
    let f = std::env::temp_dir().join(format!("lmd_gate_fixture_{}.txt", std::process::id()));
    ...
    assert!(
        out.contains("GATE_FIXTURE_CONTENT"),
        "read did not render: {out}"
    );
}
...
fn gate_reread_warms_shared_cache_without_fresh() {
    ...
    // Note: with only 2 `@read`s the trailing-directive renders full (see the #[ignore]'d
    // reread test in engine.rs); 3 reads are required to observe the `[unchanged` stub.
    let f = std::env::temp_dir().join(format!("lmd_gate_reread_{}.txt", std::process::id()));
