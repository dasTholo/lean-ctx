...
**Context Engineering for AI Agents with CCP + TDD. Shell Hook + MCP Server. 78 MCP tools, 10 read modes, 95+ shell patterns, cross-session memory (CCP), LITM-aware positioning, tree-sitter AST for 18 languages. Single Rust binary.**
...
When configured as an MCP server, lean-ctx provides 78 tools that replace or augment your editor's built-in tools:
