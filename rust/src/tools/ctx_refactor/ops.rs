...
/// Run the JetBrains IDE reformat for `abs_path`/`scope` (the fallback formatter
/// when no built-in command covers the extension). Wraps `live_jetbrains_backend`
/// + `ReformatQuery`; change detection is handled by the caller via BLAKE3.
