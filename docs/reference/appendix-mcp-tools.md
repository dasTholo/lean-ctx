...
| `ctx_refactor` | LSP-backed refactoring + name_path symbol-body edits + Two-Phase rename (IDE-first, lossless headless fallback for edits; rename needs a live IDE → BACKEND_REQUIRED; `expected_hash`/`plan_hash` BLAKE3 CONFLICT guards) + move ops (`target_path` XOR `target_parent`, PathJail, JetBrains-only) + safe_delete ops (`force`, `propagate`, JetBrains-only) + **inline ops** (Two-Phase, name_path/path+line addressed, JetBrains-only → BACKEND_REQUIRED; plan_hash TOCTOU guard; force-less — inline cannot be forced → UNSUPPORTED; `keep_definition`: inline all call sites but keep the declaration; err: UNSUPPORTED, INVALID_TARGET, BACKEND_REQUIRED, CONFLICT, INDEXING, UNSUPPORTED_LANGUAGE) + **reformat** (Single-Phase, no plan_hash/preview, addressed by name_path \| path \| path+line → Symbol \| File \| Region; `optimize_imports`: also remove unused imports) | rename\|references\|definition\|implementations\|declaration\|type_hierarchy\|symbols_overview\|inspections\|replace_symbol_body\|insert_before_symbol\|insert_after_symbol\|rename_preview\|rename_apply\|move_preview\|move_apply\|safe_delete_preview\|safe_delete_apply\|inline_preview\|inline_apply\|reformat | S |
