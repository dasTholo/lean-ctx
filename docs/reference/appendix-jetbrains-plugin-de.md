# Appendix — JetBrains-Plugin (Funktions- & Verhaltens-Referenz)

> Sprache: Deutsch. Code, Parameter, Endpunkt-/Tool-Namen und Error-Codes bleiben
> englisch. Autoritative Quellen: `packages/jetbrains-lean-ctx/src/main/kotlin/com/leanctx/plugin/…`
> (Plugin), `rust/src/lsp/backend.rs` · `rust/src/lsp/jetbrains_backend.rs` ·
> `rust/src/lsp/router.rs` · `rust/src/lsp/edit_apply.rs` (Rust-Backend),
> `rust/src/tools/registered/ctx_refactor.rs` (MCP-Tool-Schema).

Diese Referenz beschreibt das **ausgelieferte** JetBrains-Plugin: alle Funktionen,
die `ctx_refactor` über die laufende IDE (bzw. den headless-Fallback) bereitstellt
— jeweils gleichwertig aus **Agenten-Sicht** (`ctx_refactor`-Action) **und** als
roher **HTTP-Endpunkt** des Plugins, samt Verhaltensgarantien.
...
| Conflict-Guard | keiner | BLAKE3 `expected_hash` (Edits) / `plan_hash` (Rename), Rust-zentral |
...
| **`BACKEND_REQUIRED`** | `rename_preview`, `rename_apply` | `Err` — keine headless-Usage-Suche möglich |
...
### 2.5 Refactoring — Rename (Two-Phase)

**Action:** `rename` (`new_name`)
**Endpunkte:** `POST /renamePreview` → `POST /renameApply`

**Was es tut:** Benennt ein Symbol projektweit um. Intern **zweiphasig**: Phase 1
(`/renamePreview`) sammelt alle Verwendungen (`usages`) und Konflikte
(`conflicts`) und bildet daraus den `plan_hash`; Phase 2 (`/renameApply`) führt die
Umbenennung als **eine** Multi-File-Transaktion aus (ein Undo-Eintrag).
...
**Verhalten:** `BACKEND_REQUIRED` ohne laufende IDE (keine verlustfreie headless-
Usage-Suche). Bestehen Konflikte und ist `force=false`, blockiert das Gate mit
`CONFLICT`. Zwischen Preview und Apply schützt der `plan_hash` (BLAKE3, Rust-zentral)
gegen TOCTOU-Drift.

---

## 3. Verhaltensgarantien & Guards

### 3.1 BLAKE3-Conflict-Guard (Rust-zentral)

Der `expected_hash` (Edits) bzw. `plan_hash` (Rename) ist ein **BLAKE3-Hex**
...
### 3.2 Smart-Mode, Sprache, PathJail
...
- **PathJail:** Jede Datei-Operation wird vor Ausführung gegen den `project_root`
  ...
  zurückgegebene `usage`-/`changed_path` (Rename).

### 3.3 Idempotenz & Atomarität
...
| Rename | Multi-File-`WriteCommandAction` | per `plan_hash` geschützt |
...
sofortiges Rewarm; bei Multi-File-Rename wird je `changed_path` mtime-geprüft.
...
| `FILE_NOT_FOUND` | Datei nicht lesbar | Rust (`edit_apply`) | Pfad mit `ctx_tree` verifizieren |
...
| `NO_SYMBOL` | `name_path` trifft 0 Symbole | Rust | Name/Pfad korrigieren |
...
| `BACKEND_REQUIRED` | Rename ohne laufende IDE | Rust (Trait-Default) | IDE mit offenem Projekt starten |
| `INTERNAL` | sonstiger Fehler / Parse | beide | `message` prüfen; ggf. Bug melden |

---

## 6. End-to-End-Beispiele
