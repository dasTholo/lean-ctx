...
| 6 | UNSUPPORTED_LANGUAGE | `{"action":"rename_preview","name_path":"notes","new_name":"x"}` (Ziel in `notes.txt`) | `UNSUPPORTED_LANGUAGE`, kein Crash Über den `path:"notes.txt"`+`line:1`-Fallback (Rust `resolve_rename_target`, `ctx_refactor.rs:347-368`); nach Fix B kommt `UNSUPPORTED_LANGUAGE` zuverlässig vor `NO_SYMBOL`. |
