...
| 4b | Konflikt mit force | wie 4a + `"force":true` | `CONFLICT` (non-destruktiv). `force` überschreibt Symbol-/Usage-Konflikte, aber **nicht** eine Datei-Kollision: der Rename würde `Widget.kt → Gadget.kt` umbenennen und die existierende `Gadget.kt` überschreiben → `ERROR: CONFLICT: target file 'Gadget.kt' already exists`, **kein** modaler Overwrite-Dialog, kein Hang. Pre-Check `SymbolRefactorer.fileRenameCollision` (Plugin), gegen den headless-deadlock des `performRefactoring`-Overwrite-Modals. |
...
| 6 | UNSUPPORTED_LANGUAGE | `{"action":"rename_preview","path":"notes.txt","line":1,"new_name":"x"}` (Ziel in `notes.txt`) | `UNSUPPORTED_LANGUAGE`, kein Crash. **Wichtig:** den `path`+`line`-Fallback nutzen (Rust `resolve_rename_target`, `ctx_refactor.rs:347-368`), **nicht** `name_path:"notes"` — letzteres läuft in den Symbol-Resolver und liefert `NO_SYMBOL`. Nach Fix B kommt `UNSUPPORTED_LANGUAGE` zuverlässig vor `NO_SYMBOL`. |
