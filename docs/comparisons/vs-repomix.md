...
lean-ctx goes further by providing a complete context engineering layer — compression is just one of 79 tools. If you use AI coding agents daily and want persistent memory, shell compression, semantic search, and real-time observability, lean-ctx is built for that workflow.
