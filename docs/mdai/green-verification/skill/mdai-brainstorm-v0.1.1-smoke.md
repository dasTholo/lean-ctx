...
- `@set + @foreach + {{var}}` inside `@define` body iteriert nicht bei call_macro-Zeit — Workaround: statische Checklisten (T7, T13, T14).
- `@if file.containsLine "{{var}}" "..."` inside `@define` interpoliert `{{var}}` nicht — gilt für spec-reviewer.md §5 (dokumentiert als by-design conditional-prose).
- `file.containsLine` auf Pfaden außerhalb des Projekt-CWD blockiert (engine policy) — detect_mai_hook_version false-negative auf v0.x branch.
- `${MDAI_LIBRARY_ROOT}` wird bei standalone resolve_phase nicht expandiert; korrektes Verhalten beim consumer-Aufruf (write-outputs L1 include).
...
- **Engine-Limits**: @set+@foreach inside @define (siehe Diagnose-Notes) — Workaround stable, eventuell upstream-PR an markdownai-engine.
