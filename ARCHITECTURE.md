...
flowchart TB
    ...
    subgraph dispatch [Dispatch and Governance]
        ...
        HybridDispatch["Hybrid Dispatch — Context Server (69 tools)"]
        ToolRegistry["ToolRegistry — 69 trait-based tools (McpTool)"]
    ...
    HybridDispatch -->|"registry (69 tools)"| ToolRegistry
...
flowchart TD
    ...
    DispatchCall["dispatch_inner — ToolRegistry lookup (69 tools)"]
...
| `server/registry.rs` | `ToolRegistry` — HashMap-based tool lookup, `build_registry()` registers all 69 trait-based tools |
| `server/dispatch/mod.rs` | Registry dispatch — `dispatch_inner` resolves every tool through the `ToolRegistry` (69 tools); `ctx_call` routes meta-invocations back through it |
